...
function RegexFilter:__init(filter)
  assert(filter.regex, 'prefilter with type `regex` has no regex list')
  if type(filter.regex) == 'string' then
    filter.regex = {filter.regex}
  end

  assert(filter.value, 'prefilter with type `regex` has no value')

  self.__base.__init(self, filter)

  local engine = filter.engine or 'default'

  engine = assert(ENGINES[engine], string.format('prefilter with type `regex` has unknown engine %s', tostring(engine)))
